...
- **アルゴリズム(algorithm)**：確実に正しい答が出せる方法
  - 難解だったり、時間がかかることが多い
  - アルゴリズムが存在するかどうかがわからない場合も少なくない
- **ヒューリスティック(heuristic)**：たいがいうまくいく方法
...
- トヴェルスキーとカーネマン(Tversky & Kahneman, 1973)
  - 「英語の単語のなかで、kで始まる単語と3番目の文字がkの単語とは、どちらが多いか？」
    - 多くの人がkで始まる単語と答える
    -  3番目の文字がkは、kで始まる単語の3倍多い
- **可用性ヒューリスティック**：思い出すことのできた事例の数を基にして実際に存在する事例の数を推測した
...
- A「銀行の窓口係」or B「女性解放運動に熱心な銀行の窓口係」
  - トヴェルスキーとカーネマンの実験では、89%の実権参加者がBと答えた(Tversky & Kahneman, 1983)
  - BはAの部分集合
- 「リンダは女性解放運動に熱心な銀行の窓口係である」
  - **連言命題(conjunctive proposition)**：「女性解放運動に熱心」 and 「銀行の窓口係」
  - **連言錯誤(conjunction fallacy)**：連言命題の確率が個々の命題の確率を上回ってしまう錯誤
- **代表性ヒューリスティック**
...
- **プロスペクト理論(prospect theory)**
  - 5つの仮定
    - 主観的価値は客観的価値とは比例しない
    - 主観的価値の軸と客観的価値の軸は**基準点(reference point)**で交わる。
      - 基準点を境にして利得と損失にわかれる
...
- 情報伝達のために媒体が必要
  - ミツバチの「８の字ダンス」→花のある方向とそこまでの距離を他のミツバチに伝える(von Frisch, 1967)
  - 人間の言語→「音声」
  ...
  - 情報処理が系列的になる
    ...
    - 音声を構造化する規則が必要
      ...
      - 音声→個々の音声間の意味的な関係をう伝えるためには構造化するための独自の規則が必要
        - **文法(grammar)**あるいは**統語規則(syntactic rule)**
...
- 動物の音声は人間の言語と同じか？
- 人間の言語の特性→ホケット(Hockett, 1960)のリストがよく引用されてきた
  - **恣意性(arbitrariness)**
...
- 1950年代ごろ、ミラーが**心理的実在性(psychological realty)**を検証
  - 実際に人間が言語学者チョムスキーの理論のように文を処理しているか？
  - 心理言語学の出発点
- 心理言語学
  - 言語学者は自分自身の**言語的直感(linguistic intuition)**を頼りに言語を分析することが多い
...
- 言語学では文が**句構造(phrase structure)**を持っていると考える事が多い
  - 句構造の図→**構文解析木(parse tree)**あるいは**木(tree)**
...
- **1語発話(one-word utterance)**：生後1年くらいから１つの単語だけの大人が理解できる言葉を発するようになる
- **2語発話(two-word utterance)**：1歳半くらいから２つの単語をつなげるようになる
  - 2語の関係は様々
  - その後１つの発話に含まれる単語の数は増えていくが、**機能語(function word)**が欠けている。
...
- **語彙爆発(word spurt)**：憶えるペース→月に数語ほどだったのが、1歳半を過ぎる頃から毎月数十語になる
  - 語彙爆発が起きる前は発話に出てくる単語はせいぜい５０語から１００語どまり
  - 幼児は単語だけを発しているわけではない
...
- 心理言語学では外国語と言わず**第二言語(second language)**という
...
- 言語の習得には**臨界期(critical period)**があるのではないか(臨界期仮説)
  - 子どもは特に意識せずに言語を習得してしまう
  - 水鳥の**刷り込み(imprinting)**→1回の経験で成立し、効果は半永続的(Lorenz, 1952)
...
- 幼児期に第二言語の学習を行うと２つの言語を並行して学習することになる
  ...
  - 二つの言語を習得することができるかどうかは環境にかなり依存するようである
    ...
    - 家庭の中でも外でも２つの言語を使っている環境では混同が生じやすい(Reich, 1986)
    - ２つの言語が区別しやすいように注意すると混じらなくて済むという報告もある(McLaughlin, 1984)
...
- バイリンガルのほうが知能テストの成績は高かった(Peal & Lambert, 1962)
  - 異論：この研究で選ばれたバイリンガルの子どもたちはもともと知能が高かったから両方共上手に使いこなせるようになったのではないか
    - 一定期間同じバイリンガルの小学生を繰り返しテストすると、２つの言語を両方ともうまく使える小学生ほど知能テストの成績はよかった(Hakuta & Diaz, 1985)
...
- **言語相対性仮説(linguistic relativity hypothesis)**
  - この説によると思考というのは頭の中で声に出さずに言語を操作すること
  - この説が正しいとすれば、違う言語では思考や認知の内容も違っている
  - 世界の見え方は言語によって異なる相対的なもの
  - 主唱者の名前を取って**サピア-ウォーフ仮説(Sapir-Whorf hypothesis)**、単に**ウォーフ仮説**とも。
...
- 言語相対性仮説において最も多くの研究がなされてきたのは**基本色名(basic color terms)**
  ...
  - 世界中の98種類の言語で基本色名は最も多い言語には11語、最も少ない言語には2語しかない(Berlin & Kay, 1969)
    - ニューギニア高地のダニ族は白っぽい明るい色の「ミリ」と黒っぽい暗い色の「モラ」の２語だけ
...
- 色以外の最近の実験：オランダ vs テネハパ(Levinson et al., 2002)
  ...
  - テーブルに３つの物体が置いてある。→後ろを向いて先程と同じようにテーブルに並べてもらう
...
- ハイダーの研究では何の影響もないという結論になっていた
  - 30年ほど後にハイダーと同じ方法で実験を行ったところ、色の記憶には基本色名の影響が多少みられた(Roberson, Davies, & Davidoff, 2000)
    ...
    - １つの色を5秒見せる→30秒経ってから40種類の色を一度に見せてその中から最初に見せた色を選ぶ
...
- 反事実的な思考の能力は常に中国人・日本人よりもアメリカ人のほうが勝っていた(Bloom, 1981, 1984)
  - 「西欧の言語は**反事実的条件法(couterfactual)**を表すための特別な文法的な仕組みを持っているが、中国語・日本語にはそれがない」
...
- **言語理解(language comprehension)**と**言語産出(language production)**
  - I/O

#### 12-1-1. 音声と音素

- **音素(phoneme)**：ある言語のなかで同じ音に聞こえる音
  - スラッシュで囲んで表記
    - 井戸→ /i/, /d/, /o/という3つの音素
- **音声(speech sound)**は千差万別
  ...
  - 私達は物理的にはかなり違った特性を持ついろいろな音声を１つの音素として知覚している



#### 12-1-2. カテゴリー知覚

- **カテゴリー知覚(categorical perception)**：音素の知覚はカテゴリー的である
  - 物理的特性として**有声開始時間(voice onset time:VOT)**が変化する場合
    - 音素には声帯の振動の有無で**有声音(voiced sound)**と**無声音(unvoiced sound)**の2種類
      - プの/p/は振動を感じない
      - プと発音すると発声が始まってから少しだけ遅れて声帯が振動する
        - これが有声開始時間
      - ブと発音すると発音が始まると同時に声帯が振動する
      - つまり、VOTが短いと「ブ」、長いと「プ」になる
    - VOTの長さだけを変化させた音声を作り、それが/b/に聞こえるか/p/に聞こえるかを調べるという実験(Lisker & Abramson, 1970)
...
- 直感的にはボトムアップ処理：音素の認知→単語を認知→文を認知
- 実際にははっきり分かれて聞こえているわけではない
  - 会話の単語をバラバラに切り離して不規則に聞かせると、物理的な特性は変わっていないにもかかわらず、半分ぐらいしか聞き取ることができなかった(Pollack & Pickett, 1964)
...
- 単語だけでなく、音素を聞き取る際にも文脈情報は大きな役割を果たす
  ...
  - **調音結合(coarticulation)**：音素の前後に来る音素によって、含まれる周波数も時間的変化の仕方も変わってしまう
    - diとdu→どちらも/d/を含む
...
- **パンデモニアム(pandemonium: Selfridge, 1959; Selfridge & Neisser, 1960)**
  - 並列処理モデル
  - 役割分担したデーモンが協調&競争→文字を認知
...
![相互活性化モデル](img/interactive-activation-model.png)

- **相互活性化モデル(interactive activation model)**
  - パンデモニアムと同じ並列処理にトップダウン処理を加味したコネクショニスト・モデル(McClelland & Rumelhart, 1981)
...
- 言語処理システムは逐次的な処理をしながら並列的にも処理をしているらしいということがわかってきた
  ...
  - 別の単語を読む音声の提示の途中で視覚的な**語彙性判断課題(lexical decision task)**
...
- **トレース・モデル(TRACE model: McClelland & Elman, 1986)**
  - ３つの階層
  ...
  - 活性化と抑制のプロセス→**閾値(threshold)**を超えると単語が認知される
    - このプロセスは数十分の1秒で完了する
  - トレースモデルの特徴
...
- 構文解析木で表されている関係を理解するためには文法的な知識が必要
  ...
  - **構文解析(syntactic analysis, parsing)**：統語的な情報とそれを使用するための文法規則を使って関係を理解する
  - チョムスキーに始まる**生成文法(generative grammar)**では、統語情報に**句構造規則(phrase structure rule)**を適用することによって関係を理解すると想定している
...
- **遅い閉鎖(late closure: Kimball, 1973)**
  - 句を閉じるのをできるだけ遅らせようとする方略
  - 分析中の句にできるだけたくさんの単語を取り込んでしまおうという方略
  - **袋小路文(garden-path sentence)**：最初の解釈がうまくいかず、途中で解釈をやり直さなければならない文
...
- チョムスキーの生成文法ではこうした統語解析は独立した**モジュール(module)**によって行われていると想定されていた(Chomsky, 1980; Fodor, 1983)
  - モジュール：特定の情報処理に特化した分析機構
...
- 後の研究によって、文法的な知識ではない一般的な知識も統語解析に影響する場合のあることがわかってきた。
  - 2種類の文の比較(Trueswell, Tanenhaus, & Kello, 1993)
...
- 談話(discourse)やテキスト(text)
  - 文法的な知識や規則は１つの文の中で単語と単語の関係を理解するためのもの
  - 複数の文と文の間の関係を理解するのはもっと一般的な知識や論理に基づいた**推測(inference)**が必要
- 代名詞以外にも様々な推測を行っている。
  ...
  - Ed wanted an alligator for his birthday. The alligator was his favorite present.
    ...
    - 文を読むのが平均して130msecぐらい長くかかる(Haviland & Clark, 1974)
...
- 言語理解には作動記憶が大きな役割を果たしていると考えられる
  - 作動記憶の容量には個人差がある
- 作動記憶の容量を測定するために**リーディング・スパン・テスト(reading span test)**が開発された(Daneman & Carpenter, 1980)
  - 二重課題法
    - 文の音読と単語の記憶
  - 文の数が６つになると全問正解できる人はほとんど皆無になる
- リーディング・スパンは大きいほど読解能力が高くなることがわかっている(Danemna & Carpenter, 1980)
...
- 言語学者のフロムキン(Fromkin, 1971)は言語産出が6段階から成り立っていると考えた
  - (1)意味の同定
    - どういう意味を伝えるのか決める
  - (2)構文構造の選択
    - 統語的な構造を決める
  ...
  - (5)接辞と機能語の形成
    - 機能語(function word:冠詞や接続詞など)および接辞(affix: 過去形の-edなど)を追加
  - (6)音声要素の特定
...
- コネクショニスト理論→並列分散処理モデルが提案されるようになった
  - あるモデルは発話のための情報処理は4層の神経回路網によって行われていると想定(Dell, 1986)
    - 意味、統語、形態素、音素という４レベル
...
- **分散認知(distributed cognition)**：認知活動は個人と社会環境が分かちもっているものだという考え
  ...
  - **状況的認知(situated cognition)**：その場の状況によって変化する認知のあり方
...
- 帰属理論では**割引原理(discounting principle)**という方法が提案された
...
- **「人か状況か論争」(person-situation debate)**
...
- **外国語副作用(foreign language side effect)** ：慣れない外国語を使う時に思考力が低下する現象
...
- 日本人留学生を対象に実際におこなった実験(Takano & Noda, 1993)
...
- 外国語副作用について、外国語の難しさと外国語副作用とを混同する批判
  ...
  - 関連はあるが、別個の現象
- バイリンガルの知能に関する議論と混同されることもある
  - バイリンガルは知的能力が低いという昔の通説と混同し、外国語を学習すると知的能力が低下する現象だと誤解されることがある
  - 思考力の低下は一時的に起こる現象
- 外国語が十分に熟達し、注意資源の消費量が減れば、外国語副作用は消失する
...
- 母語が英語以外の人は外国語の難しさによる不利に加えて思考力が低下するというハンディも背負う
  - 外国語が母語と似ていない場合ほど外国語副作用は大きなる(Takano & Noda, 1995)
    - 外国語が英語の場合、ドイツ人と日本人では、日本人の方が大きくなる
    - 英語が国際語→インド・ヨーロッパ語族の言語以外の人々はさらに不利になる

#### 15-4-3. 外国語副作用への対策

- 特定の言語と似ているということがない人工言語→国際語
  - 作成はともかく、普及が困難
- 英語を第二公用語
  - 全国民の習熟が困難、英語ができない人が不利になる
- コンピュータを使った自動翻訳の技術を実用化すること
  - 最も実現性が高いかもしれない
  - インド・ヨーロッパ語族に属さない国々は国家戦略の目標にしてもいい
- 個人的な対策
  - 英語の学習
    - この対策は費用・時間の犠牲が大きい
  - 通訳
